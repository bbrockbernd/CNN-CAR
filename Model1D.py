...
class HyperModel1D(kt.HyperModel):
    ...
    def build(self, hp):
        ...
        n_filters_2 = hp.Int('filter count 2', 8, 128, 8, parent_name='Enable 2nd Conv', parent_values=[True])
        kernel_size_2 = hp.Int('kernel size 2', 3, 17, 2, parent_name='Enable 2nd Conv', parent_values=[True])
        drop_2 = hp.Float('drop out 2', 0.6, 1.0, 0.2, parent_name='Enable 2nd Conv', parent_values=[True])
        pool_2 = hp.Choice('max pool 2', [2, 4, 8, 16], parent_name='Enable 2nd Conv', parent_values=[True])


        dense_size_1 = hp.Int('Dense size 1', 60, 200, 20)
        drop_3 = hp.Float('drop out 3', 0.6, 1.0, 0.2)

        second_dense = hp.Boolean('Enable 2nd Dense')
        dense_size_2 = hp.Int('Dense size 2', 60, 200, 20, parent_name='Enable 2nd Dense', parent_values=[True])
        drop_4 = hp.Float('drop out 4', 0.6, 1.0, 0.2, parent_name='Enable 2nd Dense', parent_values=[True])
